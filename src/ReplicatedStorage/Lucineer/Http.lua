...
function Http.encode(t: table): string
...
function Http.decode(s: string): table
...
function Http.headers(): { [string]: string }
    return {
        ...
        ["X-Lucineer-Key"] = Config.AUTH_KEY,
    }
end
...
function Http.request(url: string, method: string, body: table?): (table?, string?)
...
function Http.get(path: string): (table?, string?)
    return Http.request(Config.WORKER_URL .. path, "GET")
end
...
function Http.post(path: string, body: table): (table?, string?)
    return Http.request(Config.WORKER_URL .. path, "POST", body)
end
